...
every annotated function and runs them. A test passes if it returns without panicking.
...
allocation-happy as it likes without anyone paying for it at run time — and
...
reason — an index out of bounds, an `unwrap` on a `None` — so the test goes on
passing after the behaviour it was checking has gone.
...
:::note
A `Result`-returning test cannot also be `#[should_panic]`. The two ask
different questions — "did it return `Err`" and "did it abort" — and the
compiler rejects the combination rather than guessing.
:::
...
private functions, private fields and `pub(crate)` items. That is the right tool for a tricky
internal — a parser state machine, a hash mixing function — where testing
through the public API would take twenty lines of setup to reach one branch.

### Integration tests, in `tests/`

Every file in `tests/` is an **integration test**, compiled as its own crate and
linked against your library the way any user's crate would be. No `#[cfg(test)]` needed — the whole
file is only built by `cargo test`.
...
`tests/common/mod.rs`, not `tests/common.rs` — because every top-level file in
`tests/` is compiled as a test crate of its own, and a bare `common.rs` would be
built as one and reported as an empty test binary.
:::
...
visible to the others. Shared mutable state has to be avoided, not scheduled
around — the ordering is not stable between runs.
...
`#[ignore]` skips a test by default and is the honest home for the slow ones —
better than commenting it out, because it is still compiled and still shows in
the summary.
...
```ignore     do not even compile — a last resort; say why
...
for something a macro needs to name but no human should call. It is a
...
for itself. It finds the empty string, the lone newline and the four-byte
emoji — the cases you were never going to think of.
