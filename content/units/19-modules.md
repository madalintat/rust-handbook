...
| **crate** | one unit of compilation — what `rustc` is invoked on, once | at most one library, any number of binaries | `src/lib.rs`, `src/main.rs`, `src/bin/*.rs` |
...
`main.rs` depends on the library exactly like any external user does — by name,
...
no `mod` names is not compiled at all — it is dead text that Cargo ignores
silently.
...
side, so a file explorer sorts them together and a module's own code is not
buried in a `mod.rs` among nine other `mod.rs` files.
...
in that module **and in all of its descendants** — never in its parent, never in
a sibling.
:::
...
controls who can read, write, and — importantly — **construct with a struct
literal**. A single private field means outside code must go through your
constructor, which is how a type keeps an invariant.
...
affect compilation order, and does not change what is compiled — the whole crate
is compiled either way. It is purely a shorthand, and two `use` lines that bring
...
is yours. `pub use` re-exports the name — anyone who can see your module can now
...
user writes — your `pub use` lines, not your directory layout.
...
`Document` from `parse` to `model` and update one `pub use` — no user notices,
...
The **prelude** is a small set of names — `Option`, `Result`, `String`, `Vec`,
`Box`, `Clone`, `Iterator`, `Drop` and a few dozen more — that the compiler
injects into every module. That is why `Option` needs no `use` and `HashMap`
...
Libraries sometimes ship their own — `use rayon::prelude::*;` — which is a
...
- **one `Cargo.lock`** — every member resolves to the same version of every dependency, so there is exactly one `serde` in the build
- **one `target/`** — a dependency compiled for `core` is not compiled again for `cli`
...
is impossible — crates form a DAG — so the split has to follow a real layering.
...
| adding a private field to that struct | also yes — same reason |
...
changes — the restriction is on *other* crates only.
:::
