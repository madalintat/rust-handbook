...
blurb: Vec, String, HashMap, VecDeque, BTreeMap — what each one costs, when it reallocates, and why the entry API is the best thing in the standard library.
---

%% A collection is a decision about cost. Every one of them makes some operation fast by making another one slow, and the whole skill is knowing which trade you just signed. Rust makes that trade legible: the types are thin, the allocations are visible, and nothing happens behind your back.
...
A `Vec<T>` is exactly three machine words on the stack. Nothing else.
...
An empty `Vec` does not allocate — it does not yet know it will be used. The
first `push` calls the allocator.
...
If you cannot tolerate one push in a thousand taking a millisecond — an audio
callback, a control loop — that average is not good enough for you, and the fix
is `with_capacity`.
:::
...
One `malloc` instead of log₂(n) of them, and no copying at all — the cheapest
performance win in Rust, and most code leaves it on the table. `collect()`
...
invalidation — and a segfault if you do not. Here it is `error[E0502]`, at
compile time.

:::gotcha
The borrow checker is not being clever about reallocation. It does not know
whether `push` will reallocate — it forbids the pattern outright, because
`push` takes `&mut self` and a shared borrow was live.
...
If order does not matter — a set of active connections, a pool of workers —
`swap_remove` turns an O(n) operation into two pointer writes. Removing 10,000
...
`s` is 6 bytes for 5 characters — `é` takes two — so byte 1 is half a character.
There is no useful thing for `s[1]` to return, so the operation does not exist
...
and looks only in that bucket. Lookup does not depend on how many entries exist
— that is the O(1).
...
and adds one. One hash, one lookup, no `unwrap`.

| method | when to use it |
|---|---|
| `or_insert(v)` | the default is cheap to build |
| `or_insert_with(make)` | the default allocates — `make` only runs on a miss |
| `or_default()` | `V: Default` — 0, empty `Vec`, empty `String` |
...
map, several cache-friendly comparisons instead of one hash — and you get things
a `HashMap` structurally cannot give you:
...
`insert` returns `bool` — `true` if the value was new. That one return value is
a complete deduplication check, no second lookup.
...
O(1) at both ends. The cost is that the elements are not one contiguous run, so
there is no `&[T]` view of the whole thing (`make_contiguous` will build one, by
moving elements).
...
|---|---|---|---|---|
| push/pop back | O(1) amortised | O(1) amortised | — | — |
| push/pop front | O(n) | O(1) amortised | — | — |
