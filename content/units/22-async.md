...
blurb: A future is inert. Nothing runs until something polls it. That one sentence explains async Rust, and it is the one thing a JavaScript promise does not do.
---
...
|---|---|---|
| memory each | 8 KB to 2 MB of stack, committed | tens to hundreds of bytes |
| 10,000 of them | gigabytes of address space | a few megabytes |
| switching between | kernel context switch, ~1 to 2 µs | a function call |
...
happens. Nothing ran, nothing was sent, and your only warning is `#[must_use]`.
...
So how does anyone know when to poll again? That is what `Context` is for. It
carries a **waker**. Before returning `Pending`, a future
...
Nothing spins in the meantime. A parked task costs its own memory and not a
cycle of CPU.

### Rust ships no executor

`std` has `Future`, `Poll`, `Context`, `Waker`, `async` and `.await`. What it
lacks is anything that runs them, so you choose an **executor**: tokio (dominant,
batteries included), or smol or async-std, or write your own.

This is deliberate and it is the same reasoning as the rest of the language. An
executor needs a thread pool, an I/O reactor and a timer. A microcontroller has
none of the three, and neither does a kernel or a browser's WASM sandbox, all of
which run async Rust today. Baking one into `std` would have made the language's
...
`join!` polls every future in one task, in turn, on one thread. That is
concurrency without parallelism. For actual parallelism across cores, `tokio::spawn` each
...
returns on the first to finish, then drops the rest. Timeouts and cancellation
are both built out of it.
...
- **why holding a `MutexGuard` across an `.await` is a bug**: the guard becomes a
  field and the lock stays held while the task is parked, possibly for milliseconds
...
That is fine while it sits still. Move it (by value, into a `Vec`, into a `Box`)
and the internal pointer still aims at the old address. That would be a dangling
pointer produced by safe code, which the language does not permit.
...
object-safe. There is no `Box<dyn Store>` without help, and the trait cannot
express `Send` bounds on the returned future. Public libraries still reach for the
...
one non-`Send` value deep inside can break `tokio::spawn` at the top. It is
better in another. Because a future is inert and cancellable, the composition
(`timeout`, `select!`, structured shutdown) beats anything you can do with a
promise.
