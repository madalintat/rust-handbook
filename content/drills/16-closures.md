...
`FnMut`, and `FnMut` implies `FnOnce`, since anything callable repeatedly is
callable once.
...
Send + 'static`, and `move` helps with the `'static` half by removing the
borrowed locals, not with the `FnOnce` half.
...
C fails because the closure captures. D fails because `dyn Fn` is unsized, and
